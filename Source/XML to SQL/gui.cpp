...
void gui::Render() noexcept
{
    ...
    ImGui::Begin("Trickster Myshop XML to SQL Converter", &isRunning, &isRunning, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);
    static bool executed = false;
    if (!executed) 
	{
        AddLog("=======================================");
        AddLog("Trickster Myshop XML to SQL Converter");
        AddLog("=======================================");
        AddLog(" ");
        AddLog("Reading libcmgds_e.xml...");
        helper->populateVectorsFromXML();
        AddLog("Generating 1 - tbl_goods.sql...");
        helper->generateSQLFiles(0);
        AddLog("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
        AddLog("Generating 2 - tbl_goods_list.sql...");
        helper->generateSQLFiles(1);
        AddLog("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
        AddLog("Generating 3 - tbl_goods_limit.sql...");
        helper->generateSQLFiles(2);
        AddLog("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
        AddLog("All done!");
        AddLog("The SQL files are inside Output folder");
        AddLog("You MUST run them in the correct order!");
        AddLog("=======================================");
        AddLog(" ");
